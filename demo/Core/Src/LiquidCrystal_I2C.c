...
static void lcd_send_cmd (LiquidCrystal_I2C *lcd, uint8_t cmd)
{
  uint8_t data_h, data_l;
	...
	data_h = (cmd&0xf0);
	data_l = ((cmd<<4)&0xf0);
	data_t[0] = data_h|(LCD_BACKLIGHT|LCD_RS_CLEAR|LCD_EN_SET); //0xXXXX.1100  en=1, rs=0
	data_t[1] = data_h|(LCD_BACKLIGHT|LCD_RS_CLEAR|LCD_EN_CLEAR); //0xXXXX.1000  en=0, rs=0
	data_t[2] = data_l|(LCD_BACKLIGHT|LCD_RS_CLEAR|LCD_EN_SET); //0xXXXX.1100  en=1, rs=0
	data_t[3] = data_l|(LCD_BACKLIGHT|LCD_RS_CLEAR|LCD_EN_CLEAR); //0xXXXX.1000  en=0, rs=0
...
static void lcd_send_data(LiquidCrystal_I2C *lcd,uint8_t data)
{
	...
	data_h = (data&0xf0);
	data_l = ((data<<4)&0xf0);
	data_t[0] = data_h|(LCD_BACKLIGHT|LCD_RS_SET|LCD_EN_SET); //0xXXXX.1101  en=1, rs=1
	data_t[1] = data_h|(LCD_BACKLIGHT|LCD_RS_SET|LCD_EN_CLEAR); //0xXXXX.1001  en=0, rs=1
	data_t[2] = data_l|(LCD_BACKLIGHT|LCD_RS_SET|LCD_EN_SET); //0xXXXX.1101  en=1, rs=1
	data_t[3] = data_l|(LCD_BACKLIGHT|LCD_RS_SET|LCD_EN_CLEAR); //0xXXXX.1001  en=0, rs=1
	HAL_I2C_Master_Transmit (lcd->pI2c, lcd->Addr,(uint8_t *) data_t, 4, 100);
}
...
{
	uint8_t data_t=LCD_BACKLIGHT;
	HAL_I2C_Master_Transmit (lcd->pI2c, lcd->Addr,&data_t, 1, 100);
}

void lcd_backlight_off(LiquidCrystal_I2C *lcd)
{
	uint8_t data_t = LCD_NOBACKLIGHT;
	HAL_I2C_Master_Transmit (lcd->pI2c, lcd->Addr,&data_t, 1, 100);
}
